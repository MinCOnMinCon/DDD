...
public class DiceManager : MonoBehaviour
{
    ...
    public void ConfirmDice(int diceNum, DiceType type)
    {
        ...
        {
            rollResult.Add(RollDice(weightList));
        }
        int idx;
        for(idx = 0; idx < diceList.Count; idx++)
        {
            diceList[idx].GetComponent<Dice>().DiceInit(rollResult[idx], type);
        }
        for (; idx < diceNum; idx++)
        {
            diceList.Add(Instantiate(dicePrefab, diceSpawnPos, Quaternion.identity));
            diceList[diceList.Count - 1].GetComponent<Dice>().DiceInit(rollResult[idx], type);
        }

    }

    private int RollDice(int[] weightList)
    {
